...
use super::{voice::HooverVoice, SynthHooverPatch};
...
pub struct SynthHoover {
    voices: Vec<HooverVoice>,
    pub master_patch: SynthHooverPatch,
...
impl SynthHoover {
    pub fn new(sample_rate: f32, num_voices: usize) -> Self {
        ...
        for _ in 0..num_voices {
            voices.push(HooverVoice::new(sample_rate));
        }
        ...
        Self {
            voices,
            master_patch: SynthHooverPatch::default(),
    ...
    pub fn process_stereo(&mut self) -> (f32, f32) {
        ...
        for voice in &mut self.voices {
            if voice.active {
                let (vl, vr) = voice.process_stereo();
                left_sum += vl;
                right_sum += vr;
            }
        }
        ...
        let left = mono_l * (1.0 - mix) + adel * mix;
        let right = mono_r * (1.0 - mix) + adel2 * mix;

        (left, right)
    }
}

impl crate::synth::PlayableInstrument for SynthHoover {
