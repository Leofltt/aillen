...
use super::SynthHooverPatch;

/// Upgraded stereo Hoover voice supporting unison, sub-bass, multi-mode filtering, and drive.
pub struct HooverVoice {
    sample_rate: f32,
    pub patch: SynthHooverPatch,
...
impl HooverVoice {
    pub fn new(sample_rate: f32) -> Self {
        let patch = SynthHooverPatch::default();
    ...
    /// Linear Congruential Generator for noise generator
    fn next_noise(&mut self) -> f32 {
        self.rng_state = self.rng_state.wrapping_mul(1664525).wrapping_add(1013904223);
        ((self.rng_state as f32) / (u32::MAX as f32)) * 2.0 - 1.0
    }

    pub fn set_patch(&mut self, patch: SynthHooverPatch) {
        self.patch = patch;
    ...
    /// Processes one sample frame and returns stereo output (Left, Right).
    pub fn process_stereo(&mut self) -> (f32, f32) {
        ...
        let pitch_mod = self.patch.lfo1_pitch_depth * lfo1_val * 20.0; // max +/- 20Hz wobble
        ...
        let lfo_cutoff_mod = self.patch.lfo1_cutoff_depth * lfo1_val * 1000.0; // max +/- 1000Hz wobble
        let final_cutoff = (env_cutoff + lfo_cutoff_mod).max(20.0);

        // 4. Stereo Filtering
        let (filtered_l, filtered_r) = match self.patch.filter_mode {
            1 => {
                // ZDF BandPass via BiquadFilter
            ...
            2 => {
                // Formant Vowel Filter: Map final_cutoff (20Hz to 4000Hz) to vowel morph index (0.0 to 1.0)
        ...
        (driven_l * amp_val, driven_r * amp_val)
    }
}

impl Voice for HooverVoice {
...
impl AudioNode for HooverVoice {
    /// Legacy AudioNode compatibility: returns Left channel of stereo processing.
    fn process(&mut self) -> f32 {
        let (l, _) = self.process_stereo();
        l
    }
}
